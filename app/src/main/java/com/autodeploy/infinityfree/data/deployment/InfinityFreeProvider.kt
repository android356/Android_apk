...
import com.autodeploy.infinityfree.data.local.dao.HostingConnectionDao
import com.autodeploy.infinityfree.data.security.SecureStorageManager
import java.io.InputStream

class InfinityFreeProvider(
    private val connectionDao: HostingConnectionDao,
    private val secureStorage: SecureStorageManager,
    private val ftpManager: FtpClientManager
) : DeploymentProvider {

    override val targetType: DeploymentTargetType = DeploymentTargetType.INFINITY_FREE
    override val displayName: String = "InfinityFree Hosting"

    companion object {
        private const val TAG = "InfinityFreeProvider"
    }
    ...
    private suspend fun getConfig(projectId: Long): FtpConnectionConfig? {
        ...
        val pass = secureStorage.getFtpPassword(conn.encryptedPasswordReference) ?: ""
    ...
    override suspend fun testConnection(projectId: Long): ProviderResult<String> {
        val config = getConfig(projectId)
            ?: return ProviderResult.Error("InfinityFree is not configured for this project", isAuthError = true)
    ...
    override suspend fun uploadFile(
        ...
        val config = getConfig(projectId)
            ?: return ProviderResult.Error("InfinityFree is not configured", isAuthError = true)
            ...
            is FtpResult.Success -> {
                ...
                ProviderResult.Success(
                    DeploymentVerificationInfo(
                        ...
                        verified = isVerified,
                        remoteSize = fileSize,
                        message = if (isVerified) "Uploaded and verified on InfinityFree" else "Uploaded to InfinityFree (verification pending)"
                    )
                )
            }
